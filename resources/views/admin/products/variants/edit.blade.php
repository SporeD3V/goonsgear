...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h2 class="text-lg font-semibold text-stone-800">{{ $variant->name }}</h2>
                <p class="text-[13px] text-stone-500">
                    Variant of <span class="font-medium text-stone-700">{{ $product->name }}</span>
                    &middot; <span class="font-mono">{{ $variant->sku }}</span>
                </p>
            </div>
        ...
        <div class="admin-card rounded-xl border border-stone-200 bg-white p-5 shadow-sm" data-delay="1">
            <form method="POST" action="{{ route('admin.products.variants.update', [$product, $variant]) }}" class="space-y-4" novalidate>
                ...
                <div class="flex items-center gap-3 border-t border-stone-100 pt-4">
                    <button type="submit" class="rounded-lg bg-[#36a2eb] px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-[#2b8ac9]">Save Variant</button>
