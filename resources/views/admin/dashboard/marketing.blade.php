...
<div class="admin-card rounded-xl border border-stone-200 bg-white p-5 shadow-sm" data-delay="5">
    @php
        $marketingAbandonedNoteOptions = collect($topAbandonedProducts)
            ->values()
            ->map(fn ($row, $index) => [
                'key' => 'marketing-abandoned-products::' . ($row['product_name'] ?? $index) . '::' . $index,
    ...
    @endif
    @include('admin.dashboard._contextual-notes', ['context' => 'marketing-abandoned-products', 'label' => 'Top Abandoned Products', 'anchorOptions' => $marketingAbandonedNoteOptions])
</div>
