...
<div class="grid gap-6 lg:grid-cols-2">
    ...
    <div class="admin-card rounded-xl border border-stone-200 bg-white p-5 shadow-sm" data-delay="3">
        @php
            $inventoryStatusNoteOptions = collect($productStatus)
                ->map(function ($count, $status) {
                    return [
                        'key' => 'inventory-product-status::' . $status,
                        'label' => ucfirst($status),
                        'value' => number_format((int) $count) . ' products',
                        'meta' => [
                            'status' => $status,
                            'count' => (int) $count,
                        ],
        ...
        @include('admin.dashboard._contextual-notes', ['context' => 'inventory-product-status', 'label' => 'Product Status', 'anchorOptions' => $inventoryStatusNoteOptions])
    </div>
</div>
