...
@section('content')
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.regional-discounts.index') }}" class="text-sm text-[#36a2eb] hover:underline">Back to list</a>
    </div>
    ...
    <form method="POST" action="{{ route('admin.regional-discounts.update', $discount) }}" class="grid gap-4 max-w-xl">
        ...
        <div>
            <button type="submit" class="rounded bg-stone-800 px-4 py-2 text-sm font-medium text-white hover:bg-stone-900">Save Changes</button>
        </div>
    </form>
    </div>
