...
@section('content')
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.bundle-discounts.index') }}" class="text-sm text-[#36a2eb] hover:underline">Back to list</a>
    </div>
    ...
    <form method="POST" action="{{ route('admin.bundle-discounts.store') }}" class="grid max-w-4xl gap-4">
        ...
        <div>
            <button type="submit" class="rounded bg-stone-800 px-4 py-2 text-sm font-medium text-white hover:bg-stone-900">Create Bundle Discount</button>
        </div>
    </form>
    </div>
    </div>
