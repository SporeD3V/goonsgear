    ...
    <form method="POST" action="{{ route('admin.tags.update', $tag) }}" class="space-y-4" enctype="multipart/form-data" novalidate>
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $tag->name) }}" class="w-full rounded-lg border border-stone-200 focus:border-[#36a2eb] focus:outline-none px-3 py-2" required>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium">Slug</label>
                <input type="text" name="slug" value="{{ old('slug', $tag->slug) }}" class="w-full rounded-lg border border-stone-200 focus:border-[#36a2eb] focus:outline-none px-3 py-2" required>
            </div>
        </div>
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <div>
                ...
                <select name="type" id="tag-type" class="w-full rounded-lg border border-stone-200 focus:border-[#36a2eb] focus:outline-none px-3 py-2" required>
        ...
        <div>
            ...
            <textarea name="description" rows="4" class="w-full rounded-lg border border-stone-200 focus:border-[#36a2eb] focus:outline-none px-3 py-2">{{ old('description', $tag->description) }}</textarea>
        </div>

        {{-- Logo upload: artist & brand only --}}
        <div id="logo-section" class="{{ in_array(old('type', $tag->type), ['artist', 'brand']) ? '' : 'hidden' }} space-y-3 rounded-lg border border-stone-200 p-4">
            ...
            <div>
                ...
                <input type="file" name="logo" accept="image/*" class="w-full rounded-lg border border-stone-200 focus:border-[#36a2eb] focus:outline-none px-3 py-2 text-sm">
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="rounded bg-[#36a2eb] px-4 py-2 text-white hover:bg-[#2b8ac9]">Update</button>
            <a href="{{ route('admin.tags.index') }}" class="text-sm text-stone-600 hover:underline">Cancel</a>
        </div>
    </form>
    </div>
