...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.tags.store') }}" class="space-y-4" enctype="multipart/form-data" novalidate>
        ...
        <div id="logo-section" class="{{ in_array(old('type', 'artist'), ['artist', 'brand']) ? '' : 'hidden' }} space-y-3 rounded-lg border border-stone-200 p-4">
            ...
            <label class="inline-flex items-center gap-2 text-sm">
                ...
                <span class="text-xs text-stone-400">(requires a logo to take effect)</span>
            </label>
        </div>
