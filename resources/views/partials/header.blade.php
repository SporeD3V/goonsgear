            ...
            @foreach ($headerCategories as $cat)
                @if ($cat->slug === 'sale')
                    <a href="{{ route('shop.category', $cat->slug) }}"
                       class="rounded-lg px-3 py-2 text-sm font-black uppercase tracking-wider text-black transition-all duration-200 hover:bg-slate-100"
                       title="Sale">
                        Sale
                    </a>
                @elseif ($cat->children->isNotEmpty())
                    <div class="group relative">
                        ...
                        <div class="invisible absolute left-0 top-full z-50 min-w-48 overflow-hidden rounded-xl border border-slate-100 bg-white opacity-0 shadow-xl transition-all duration-200 group-hover:visible group-hover:opacity-100">
                ...
                @foreach ($headerCategories as $cat)
                    @if ($cat->slug === 'sale')
                        <a href="{{ route('shop.category', $cat->slug) }}"
                           ...
                           class="flex items-center gap-3 rounded-lg px-4 py-3 text-base font-black uppercase tracking-wider text-black transition-all duration-200 hover:bg-slate-50">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
